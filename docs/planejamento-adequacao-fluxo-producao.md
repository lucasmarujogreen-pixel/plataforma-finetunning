...
**Status (2026-07-13, iteração 5 — em validação):** (h) **requisitos cadastrados da norma como contexto do fluxo sem requisito** — o maior gap restante é o tipo 2/3 (dev 0.5319 / test 0.42, contra ~0.73 do tipo 1). Nas análises sem requisito a norma pode ter requisitos CADASTRADOS (`normas.RequisitosIds` → `requisitos.TextoPuro`) mesmo sem análise individual: cobertura medida no universo tipo 2/3 (n=1.932) — 20,5% têm requisitos da norma, 56,8% têm Complemento, 34,1% não têm nenhum texto específico. Entram como bloco "Requisitos cadastrados para a norma (não analisados individualmente)" no `build_query_text` (embedding + alvo + few-shot, mesmo mecanismo da alavanca c), APENAS quando a análise não tem requisitos próprios — tipo 1 fica intocado (comparabilidade). Teto de 10 requisitos no pipeline (`NORMA_REQUISITOS_CAP`; mediana 7 quando existem, cauda até 1.411). Validação dev pendente; leitura será no corte tipo 2/3 do pareado (só ~20% dessas 25 amostras dev devem mudar).

**Validação dev da v1 (run `20260714_152748`): 0.6954, sem sinal de ganho na coorte-alvo.** Pareado vs a referência 0.6995 em três cortes: **COM bloco** (7 amostras tipo 2/3 cuja norma tem requisitos): 0.1667 → 0.0952, nenhuma melhorou e uma regrediu forte (análise 8202, Decreto Federal: 0.500 → 0.000 — a predição encolheu de 9 itens com 3 acertos para 1 item errado); **SEM bloco** (19 tipo 2/3, controle): -0.019 (3/2, ruído); **tipo 1** (124, controle de variância — nada mudou nele): +0.015 (10/6), que dá a régua do ruído entre rodadas. Causa provável da regressão: o bloco (até 10 requisitos × ~180 chars) **dominou o embedding** e trocou os vizinhos das 7 amostras afetadas — o mesmo vetor de dano já visto em retrieval. A coorte-alvo é estruturalmente difícil (baseline 0.17: Lei Federal, Comunicado, NR genéricas — casos em que a condição depende de contexto fora do texto). **Correção (v2): bloco fora do embedding, só no prompt** (`include_norma_requisitos` em `build_query_text`, ligado apenas por `render_target_block`) — os vizinhos voltam a ser idênticos aos da referência e a próxima rodada mede o efeito puro do texto no prompt. Se seguir neutro, a alavanca (h) é revertida — população endereçável pequena (~4,5% do universo) e sem evidência de sinal.

**Validação dev da v2 (run `20260714_154616`): 0.7057 — novo melhor dev; alavanca (h) confirmada.** Na coorte-alvo (7 amostras COM bloco, vizinhos idênticos aos da referência em todas — efeito puro do prompt): **0.1667 → 0.3810** (+0.2143; 2 melhoraram, 0 pioraram) — análise 8816 (Comunicado) foi de 0 a acerto exato, 8808 (Lei Federal) de 0 a 0.500, e 8202 (Decreto Federal) recuperou os 0.500 que a v1 havia destruído via embedding. Controles no ruído: tipo 1 +0.0172 (8/4), tipo 2/3 sem bloco -0.0418 (2/1, n=19, puxado por uma amostra). Micro tipo 2/3: 0.5319 → 0.54. Configuração vigente: iteração 3 + alavanca (h) v2 (bloco só no prompt) — pronta para consolidar no test n=200 (último número oficial, 0.6171, é da iteração 3).

**Ablação do pool da dica (run `20260714_160303`, `--hint-k 3`, única diferença vs 0.7057): 0.6823 — dica-10 inocentada.** Pareado: 7 melhores / 15 piores com dica-3, e **Resolução é a espécie que mais perde** (0.5896 → 0.4296, 1/5) — o oposto da hipótese. Com isso as DUAS causas suspeitas da regressão de Resolução no test (órgão no embedding; pool 3→10) foram refutadas em ablação isolada no dev; a explicação restante é flutuação amostral do test (n=24, pareado 2/8) — reavaliar no próximo test n=200 em vez de investigar mais no dev. Encerra a investigação; configuração vigente confirmada (dica-10 + órgão no embedding + alavanca h v2).

**Resultado test n=200 (run `20260714_163442`, pareado com `20260713_120625`): 0.6171 → 0.6236 — novo número oficial.** Decomposição honesta do ganho: (1) **a remoção do feedback demo responde por praticamente tudo** — as 5 amostras da norma 1376 (todas **Resolução**, tipo 1) subiram de 0.3933 para 0.5400 na média; a previsão feita na PDA-717 ("sem essas 5 o micro sobe para 0.6234") bateu com o medido (0.6236). (2) **O mistério da regressão de Resolução está RESOLVIDO**: a causa era o feedback demo — micro da espécie 0.3495 → **0.4124**, de volta ao nível pré-regressão (0.4112); órgão e pool da dica seguem inocentados. (3) **Alavanca (h) foi neutra no test**: a coorte com bloco tem só 5 amostras e nenhuma mudou de F1 (0.30 → 0.30) — o ganho medido no dev (+0.21 na coorte de 7) não se repetiu aqui; mantida por ser inofensiva ao retrieval (só prompt) e validada no dev, mas sem evidência no test. Tipo 1 estável (0.645 → 0.6497 micro); tipo 2/3 0.42 → 0.433. Por espécie: Resolução +0.08, Lei Complementar +0.05, Portaria +0.05, NR +0.05; Lei Municipal -0.01. Linha oficial do test: 0.5605 → 0.5960 → 0.6171 → **0.6236**.
...
## 8. Próximos passos — implementação do piloto assistido (2026-07-14)

O plano do piloto vive na POC vencedora, junto do código que ele reusa: **`poc_promptanalise/docs/piloto-assistido.md`** (escopo por espécie, fases P1–P5, guard-rails e critérios de go/no-go). A POC de fine-tuning (este repositório) não faz parte dos próximos passos — fica como registro histórico e como referência caso o gatilho de custo/latência da Fase G volte a ser discutido.
