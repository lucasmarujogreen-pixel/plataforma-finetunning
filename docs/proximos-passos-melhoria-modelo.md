...
**Atualização (10/07/2026) — a previsão de F1 0.57–0.60 se confirmou, mas não do jeito que eu esperava.** Testei `claude-sonnet-5` (API, não nuvem de treino) no mesmo protocolo few-shot do Haiku — ver Etapa 10 em `poc-pda-716.md`. Resultado: **F1 0.5731**, dentro da faixa estimada aqui, e **superando o kNN (0.559) pela primeira vez em toda a investigação**. A diferença em relação ao raciocínio original desta seção: eu tinha estimado esse teto assumindo que o ganho ficaria concentrado só na fatia <0.5 de similaridade (onde o Haiku já vencia); o resultado real sugere que o Sonnet 5 ganha o suficiente também nos casos médios pra empurrar o F1 agregado acima do kNN, não só naquele recorte — ainda não medi a estratificação por similaridade pra confirmar isso com certeza. Isso não invalida o diagnóstico estrutural (teto de retrieval, natureza repetitiva do histórico) — apenas mostra que "modelo mais forte" tem uma janela de ganho real que o Haiku, especificamente, não conseguia alcançar. Não muda a recomendação de produtizar retrieval como base, mas abre um caminho concreto de complementar com Sonnet 5 nos casos que o retrieval sozinho não resolve bem.
