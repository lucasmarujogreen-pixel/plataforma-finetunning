...
## 9. Pós-POC — Teste do reranker cross-encoder (08–09/07/2026)

> A POC acima foi formalmente concluída na seção 8. Esta seção documenta um experimento adicional, feito depois, para testar uma das hipóteses de melhoria listadas em `proximos-passos-melhoria-modelo.md` (seção 2.2b): treinar um reranker dedicado em vez de escalar o LLM generativo. Não altera a recomendação da seção 8 — reforça-a com mais evidência.

**Motivação.** A Etapa 8 mostrou que o fine-tuned v6 acerta o ramo certo da taxonomia (F1 raiz 0.739, o melhor dos três sistemas) mas erra a string exata da folha (F1 item 0.457, o pior dos três) — e que mesmo a API Haiku, ordens de magnitude mais capaz, também perde para a cópia literal do kNN (0.500 vs. 0.559). Isso indicava que o gargalo é a natureza generativa da tarefa (produzir texto livre que precisa casar caractere-a-caractere com uma string de uma taxonomia fechada de 2.715 itens), não a capacidade do modelo. Um reranker cross-encoder ataca esse sintoma por construção: ele pontua candidatos que já existem e devolve o texto do candidato vencedor — nunca gera uma string nova, então nunca pode "quase acertar" e falhar no match exato.

**Implementação.** Construído como um trilho de treino paralelo ao causal-LM existente (reuso do que já era genérico — hardware, tokenizer, experiment manager, callbacks, plots, kNN de candidatos, F1 hierárquico — e duplicação só do que é específico de classificação de pares). Modelo: `BAAI/bge-reranker-v2-m3` (~568M parâmetros, multilíngue, LoRA `task_type=SEQ_CLS`). Biblioteca: `sentence-transformers` (`CrossEncoder` + `CrossEncoderTrainer` + `losses.LambdaLoss`). Dataset: pares listwise `{query, docs, labels}` construídos a partir do mesmo retrieval kNN da Etapa 3/7, com o item gold sempre garantido entre os candidatos — 14.194 análises → 11.468 pares de treino / 613 de validação.

**Bugs reais encontrados e corrigidos durante a implementação** (nenhum seria pego só por teste unitário — só apareceram rodando o modelo real de ponta a ponta):

1. **Contaminação da query**: o dataset v6 (usado pelo fine-tuned v6) injeta um bloco "Candidatos da taxonomia" dentro do texto da mensagem `user` — usar essa fonte para construir a query do reranker vazava os candidatos dentro do próprio texto de entrada. Corrigido trocando a fonte para v5 (mesmo universo de 14.194 análises, texto limpo).
2. **Download da Hugging Face Hub travando**: o protocolo XET de transferência rápida trava indefinidamente neste ambiente sandboxed para arquivos grandes. Corrigido com `HF_HUB_DISABLE_XET=1`.
3. **Conflito PEFT em cabeça de classificação**: `target_modules="all-linear"` + `task_type=SEQ_CLS` faz o PEFT tentar aplicar LoRA e `modules_to_save` na mesma camada da cabeça de classificação, gerando `TypeError`. Corrigido com `exclude_modules`+`modules_to_save` explícitos apontando para as mesmas camadas (`classifier`, `pre_classifier`, `score`).
4. **Merge do LoRA nunca acontecia (corrupção silenciosa)**: `get_peft_model()` muta os submódulos do modelo base *in place* e devolve um wrapper externo separado; o código inicial checava `isinstance(model.model, PeftModel)` para decidir se fazia merge antes de salvar — mas `model.model` é a referência interna mutada, nunca é o wrapper externo, então a checagem sempre era falsa e o "merge" nunca executava. Os checkpoints salvos ficavam com chaves `lora_A`/`lora_B`/`modules_to_save` misturadas, e recarregar o modelo gerava um LOAD REPORT cheio de chaves inesperadas/faltantes — sem erro visível, só resultado errado. Corrigido guardando uma referência explícita ao wrapper externo (`object.__setattr__`) no momento da construção e chamando `.merge_and_unload()` nela diretamente antes de salvar.
5. **`load_best_model_at_end=True` corrompia o reload**: pelo mesmo motivo do bug 4, recarregar via `from_pretrained` fresco não entende nomenclatura de parâmetro do PEFT. Desligado deliberadamente (o `EarlyStoppingCallback` continua funcionando via `metric_for_best_model`, independente desse flag).
6. **Lentidão do treino real (23,96s/it → picos de 70s/it e subindo)**: investigado ao vivo durante o treino. Descartado overflow para memória compartilhada da GPU (o padrão documentado na Etapa 1) — VRAM dedicada estável em ~95% via `nvidia-smi`, sem crescimento, sem uso de swap do host, temperatura (58–64°C) longe de throttle térmico. Hipótese mais provável: fragmentação do alocador de memória CUDA do PyTorch, causada pelos batches de tamanho muito variável do dataset listwise (candidatos por query variam de ~3 a 40) somada à VRAM já colada no teto de 6GB — sem folga para o alocador reaproveitar blocos de forma eficiente. Treino seguiu do mesmo jeito (não haveria tempo de validar uma correção de alocador sem reiniciar o treino em andamento).
7. **Seleção fixa `top_k=5` na avaliação inflava falsos positivos**: a média real de itens gold por query no test set é 1,78 (118/200 amostras têm exatamente 1 item), mas a avaliação sempre devolvia os 5 candidatos mais bem pontuados. Isso por si só derrubava o F1 item de ~0.35 para 0.30. Corrigido para `top_k=2` (melhor valor entre os testados) em `configs/reranker/evaluation/default.yaml`.

**Resultado final** (1 época, LoRA, `loss_type=lambda`, 12h49 de GPU local, avaliado nas mesmas 200 amostras da Etapa 8 já com a correção do bug 7):
...
**Diagnóstico.** O reranker não superou nenhum dos três sistemas anteriores — nem o F1 raiz (0.603, pior que os 0.73+ dos outros dois modelos). Inspeção qualitativa das amostras mostra um padrão específico: o modelo frequentemente acerta o *tópico* certo (ex. os 5 candidatos escolhidos eram todos variantes de "Biomassa > emissões atmosféricas", a mesma família do item gold) mas erra a variante exata dentro desse tópico (datas de licenciamento, condição de dispensa, etc. — leaves quase-idênticos entre si). A curva de `eval_loss` (0.28→0.20 nos primeiros ~400/717 steps, quase estável depois) sugere sub-treino em 1 época, mas isso é ambíguo porque o *scheduler* de learning rate foi calibrado para essa única época — não é prova de que 3 épocas com *schedule* próprio chegariam ao mesmo platô.

**Decisão.** Não investir mais GPU nessa configuração específica sem antes ter evidência de que outra coisa muda o resultado (ver seção 2.2b atualizada em `proximos-passos-melhoria-modelo.md` para os próximos passos investigados a partir daqui). O código do trilho de reranker (`src/finetuning/training/reranker_*.py`, `src/finetuning/application/*_reranker_model.py`, `ft train-reranker`/`ft evaluate-reranker`) fica no repositório, testado e funcional, para retomar caso as correções identificadas depois mudem o cenário.

**Retreino com correções (10/07/2026).** A investigação de causa-raiz (seção 2.2c de `proximos-passos-melhoria-modelo.md`) confirmou cauda longa extrema nos itens da taxonomia como gargalo dominante, e duas correções de baixo risco foram implementadas: oversampling balanceado por classe (itens raros repetidos até 4x no treino) e corte de candidatos por query no treino (`max_train_docs_per_query=24`, necessário também para caber na VRAM com folga de 30%+ para outras tarefas no notebook — ver `hardware.max_vram_fraction`). O retreino sofreu uma queda de energia/crash do sistema em 99% da época (step 990/996) — recuperado via transplante manual de pesos do checkpoint-900 (`scripts/recover_reranker_checkpoint.py`; o `ft resume-reranker` automático não funciona para este LoRA setup, ver nota técnica abaixo) e finalizado a partir do checkpoint-96 dessa segunda rodada (`scripts/finalize_reranker_from_checkpoint.py`, contornando um crash de CUDA "illegal memory access" específico do loop de avaliação interna do treino).
...
| API Claude Haiku 4.5 few-shot | 0.500 | 0.732 |
| Fine-tuned v6 generativo (0.6B) | 0.457 | 0.739 |
| Reranker (1 época, sem correções) | 0.351 | 0.603 |
| **Reranker (1 época, com oversampling + doc-cap)** | **0.370** | 0.597 |
