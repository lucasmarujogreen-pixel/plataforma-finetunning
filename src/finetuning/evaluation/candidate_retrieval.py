...
Promoted from ``scripts/knn_retrieval.py`` into the installed package: the
reranker dataset builder (``scripts/build_reranker_dataset.py``) and the
reranker evaluator (``application/evaluate_reranker_model.py``) both need
this candidate-generation logic, and a module inside ``finetuning`` cannot
import from ``scripts/`` (not part of the installed package).
``scripts/knn_retrieval.py`` re-exports these names for backward
compatibility with ``build_poc_dataset_v2.py`` and existing tests.
"""
...
def interleave_neighbors(
    ...
    neighbors: list[int] = []
    seen: set[int] = set()
    for embedding_index, lexical_index in zip(embedding_row, lexical_row, strict=True):
